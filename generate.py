...
if __name__ == "__main__":
    ...
    device = torch.device(
        f"cuda:{arguments.gpu_device}"
        if torch.cuda.is_available()
        else (
            f"mps:{arguments.gpu_device}"
            ...
            torch.stack(
                [
                    ...
                    for mask, facie in zip(
                        dataset.masks_pyramid[i], dataset.facies_pyramid[i]
                    )
                ],
                dim=0,
            )
    ...
    print(
        f"Facies generated at '{os.path.join(arguments.out_path, 'generated_facie_[1, 2, ...].tif')}'."
    )
