...
class SPADEGenerator(nn.Module):
    ...
    def __init__(
        ...
        # Initial projection from conditioning to feature space
        # This replaces the head ConvBlock - we start from a learned constant
        # and modulate it with SPADE
        self.init_conv = nn.Conv2d(
            cond_channels, num_feature, kernel_size=kernel_size, padding=padding_size
        )
        ...
        prev_channels = num_feature
        for i in range(num_layer - 2):
            channels = int(num_feature / pow(2, (i + 1)))
            out_ch = max(channels, min_num_feature)
            in_ch = prev_channels if i == 0 else max(2 * channels, min_num_feature)
            
            self.spade_blocks.append(
                SPADEConvBlock(
                    in_ch, out_ch, cond_channels,
                    kernel_size, padding_size, 1
                )
            )
