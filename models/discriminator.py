...
class Discriminator(nn.Module):
    def __init__(self,img_ch=1,kernel:int=3):
        ...
        first_discriminator.append(nn.Sequential(nn.Conv2d(self.img_ch, self.nf, kernel, 1, kernel//2),
                                             nn.LeakyReLU(2e-1)))
        for _ in range(3):
            first_discriminator.append(nn.Sequential(nn.Conv2d(self.nf, self.nf, kernel, 1, kernel//2),
                                                 nn.BatchNorm2d(self.nf),
                                                 nn.LeakyReLU(2e-1)))

        first_discriminator.append(nn.Sequential(nn.Conv2d(self.nf, 1, kernel, 1, kernel//2)))
    ...
    def progress(self):
        ...
        tmp_discriminator.append(nn.Sequential(nn.Conv2d(self.img_ch, self.nf, self.kernel, 1, self.kernel//2),
                                               nn.LeakyReLU(2e-1)))

        for _ in range(3):
            tmp_discriminator.append(nn.Sequential(nn.Conv2d(self.nf, self.nf, self.kernel, 1, self.kernel//2),
                                                   nn.BatchNorm2d(self.nf),
                                                   nn.LeakyReLU(2e-1)))

        tmp_discriminator.append(nn.Sequential(nn.Conv2d(self.nf, 1, self.kernel, 1, self.kernel//2)))
