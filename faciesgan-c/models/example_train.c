...
static mlx_array *make_synthetic_grad(const mlx_array *param)
{
    ...
    free(buf);
    mlx_array *ptr = (mlx_array *)malloc(sizeof(mlx_array));
    if (!ptr)
    {
        mlx_array_free(a);
        return NULL;
...
int main(void)
{
    printf("Smoke test: train_step with synthetic gradients\n");
    ...
    if (gen_n > 0)
    {
        gen_grads = (mlx_array **)malloc(sizeof(mlx_array *) * gen_n);
    ...
    if (disc_n > 0)
    {
        disc_grads = (mlx_array **)malloc(sizeof(mlx_array *) * disc_n);
    ...
    int r = mlx_faciesgan_train_step(m, opt_g, gen_grads, gen_n, opt_d, disc_grads, disc_n);
    printf("train_step result: %d\n", r);
