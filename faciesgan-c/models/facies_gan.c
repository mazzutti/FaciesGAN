...
struct MLXFaciesGAN
{
    ...
    /* optional shapes & noise amp bookkeeping (set via set_shapes / set_noise_amps) */
    int *shapes_flat;
    int n_shapes;
    float *noise_amps;
    int n_noise_amps;
...
void mlx_faciesgan_set_use_create_graph_gp(int use) { g_use_create_graph_gp = use ? 1 : 0; }
int mlx_faciesgan_get_use_create_graph_gp(void) { return g_use_create_graph_gp; }

MLXFaciesGAN *mlx_faciesgan_create(int num_layer, int kernel_size, int padding_size, int num_img_channels, int num_feature, int min_num_feature, int discriminator_steps, int generator_steps)
{
    MLXFaciesGAN *m = (MLXFaciesGAN *)malloc(sizeof(MLXFaciesGAN));
    if (!m)
        return NULL;
    memset(m, 0, sizeof(*m));
...
int mlx_faciesgan_compute_diversity_loss(MLXFaciesGAN *m, mlx_array **fake_samples, int n_samples, float lambda_diversity, mlx_array **out_loss)
{
    (void)m;
    if (!out_loss)
        return -1;
    ...
    {
        *out_loss = (mlx_array *)malloc(sizeof(mlx_array));
        if (!*out_loss)
            return -1;
    ...
    for (int i = 0; i < n_samples; ++i)
    {
        for (int j = i + 1; j < n_samples; ++j)
        {
            ...
            int *axes = (int *)malloc(sizeof(int) * ndim);
            if (!axes)
            {
                mlx_array_free(diff);
                mlx_array_free(sq);
...
int mlx_faciesgan_compute_masked_loss(MLXFaciesGAN *m, const mlx_array *fake, const mlx_array *real, const mlx_array *well, const mlx_array *mask, float well_loss_penalty, mlx_array **out_loss)
{
    (void)m;
    if (!out_loss)
        return -1;
    ...
    if (mlx_multiply(&fmasked, *fake, *mask, s) != 0 || mlx_multiply(&rmasked, *real, *mask, s) != 0)
    {
...
MLXDiscriminator *mlx_faciesgan_build_discriminator(MLXFaciesGAN *m)
{
    ...
    m->discriminator = mlx_discriminator_create(m->num_layer, m->kernel_size, m->padding_size, m->num_img_channels);
    if (!m->discriminator)
        return NULL;
    /* create initial scale */
    if (mlx_discriminator_create_scale(m->discriminator, m->num_feature, m->min_num_feature) != 0)
    {
...
MLXGenerator *mlx_faciesgan_build_generator(MLXFaciesGAN *m)
{
    ...
    if (m->generator)
        return m->generator;
    m->generator = mlx_generator_create(m->num_layer, m->kernel_size, m->padding_size, m->gen_input_channels, m->num_img_channels);
    if (!m->generator)
        return NULL;
    /* Create a default scale 0 so parameters exist */
    if (mlx_generator_create_scale(m->generator, 0, m->num_feature, m->min_num_feature) != 0)
    {
...
int mlx_faciesgan_create_generator_scale(MLXFaciesGAN *m, int scale, int num_features, int min_num_features)
{
    if (!m)
        return -1;
    if (!m->generator)
    ...
    return mlx_generator_create_scale(m->generator, scale, num_features, min_num_features);
}

mlx_array_t mlx_faciesgan_generate_fake(MLXFaciesGAN *m, const mlx_array *z_list, int z_count, const float *amp, int amp_count, mlx_array_t in_noise, int start_scale, int stop_scale)
{
    if (!m)
        return in_noise;
    ...
    float *tmp_amps = NULL;
    if ((!use_amp || use_amp_count <= 0) && m->noise_amps && m->n_noise_amps > 0)
    {
    ...
    mlx_array_t out = mlx_generator_forward(m->generator, z_list, z_count, use_amp, use_amp_count, in_noise, start_scale, stop_scale);
...
AGValue *mlx_faciesgan_generator_forward_ag(MLXFaciesGAN *m, AGValue **z_list, int z_count, const float *amp, int amp_count, AGValue *in_noise, int start_scale, int stop_scale)
{
    if (!m)
        return NULL;
    ...
    for (int index = start_scale; index <= stop_scale; ++index)
    {
        ...
        AGValue *padded = ag_pad(ups, axes, 2, low_pad, 2, high_pad, 2, 0.0f, "constant");
...
AGValue *mlx_faciesgan_discriminator_forward_ag(MLXFaciesGAN *m, AGValue *input, int scale)
{
    if (!m || !input)
        return NULL;
...
AGValue *mlx_faciesgan_compute_gradient_penalty_ag(MLXFaciesGAN *m, AGValue *real, AGValue *fake, int scale, float lambda_grad)
{
    if (!m || !real || !fake)
        return NULL;

    /* Create random alpha with shape (batch,1,1,1) using MLX random_uniform, then wrap as AGValue (no grad) */
    mlx_array *rarr = ag_value_array(real);
    if (!rarr)
        return NULL;
    const int *rshape = mlx_array_shape(*rarr);
    int ndim = (int)mlx_array_ndim(*rarr);
    if (ndim < 1)
        return NULL;
    int batch = rshape[0];
    int ashape[4] = {batch, 1, 1, 1};
    mlx_array a_arr = mlx_array_new();
    mlx_stream s = mlx_default_cpu_stream_new();
    if (mlx_random_uniform(&a_arr, mlx_array_empty, mlx_array_empty, ashape, 4, MLX_FLOAT32, mlx_array_empty, s) != 0)
    {
        mlx_stream_free(s);
        return NULL;
    }
    ...
    AGValue *alpha = ag_value_from_new_array(&a_arr, 0);
...
AGValue *mlx_faciesgan_compute_diversity_loss_ag(MLXFaciesGAN *m, AGValue **fake_samples, int n_samples, float lambda_diversity)
{
    (void)m;
...
AGValue *mlx_faciesgan_compute_masked_loss_ag(MLXFaciesGAN *m, AGValue *fake, AGValue *real, AGValue *well, AGValue *mask, float well_loss_penalty)
{
    (void)m;
    if (!fake || !real)
        return ag_scalar_float(0.0f);
...
AGValue *mlx_faciesgan_discriminator_forward_ag_with_params(MLXFaciesGAN *m, AGValue *input, int scale, AGValue ***out_params, int *out_n_params)
{
    if (!m || !input)
    {
        if (out_params)
    ...
    AGValue **params = (AGValue **)malloc(sizeof(AGValue *) * n);
    if (!params)
    {
        mlx_discriminator_free_parameters_list(plist);
        if (out_params)
...
AGValue *mlx_faciesgan_generator_forward_ag_with_params(MLXFaciesGAN *m, AGValue **z_list, int z_count, const float *amp, int amp_count, AGValue *in_noise, int start_scale, int stop_scale, AGValue ***out_params, int *out_n_params)
{
    if (!m)
    {
        if (out_params)
    ...
    if (z_count > 0 && z_list)
    {
        zvals = (mlx_array *)malloc(sizeof(mlx_array) * z_count);
        if (!zvals)
        {
            if (out_params)
                *out_params = NULL;
    ...
    mlx_array_t numeric_out = mlx_generator_forward(m->generator, (const mlx_array *)zvals, z_count, amp, amp_count, in_arr, start_scale, stop_scale);
...
int mlx_faciesgan_optimize_generator_from_ag(MLXFaciesGAN *m, MLXOptimizer *opt, AGValue **params, int n)
{
    if (!m || !opt)
        return -1;
    if (!params || n <= 0)
...
int mlx_faciesgan_optimize_discriminator_from_ag(MLXFaciesGAN *m, MLXOptimizer *opt, AGValue **params, int n)
{
    if (!m || !opt)
        return -1;
    if (!params || n <= 0)
...
int mlx_faciesgan_train_step_from_ag(MLXFaciesGAN *m, MLXOptimizer *opt_g, AGValue **gen_params, int gen_n, MLXOptimizer *opt_d, AGValue **disc_params, int disc_n)
{
    if (!m)
        return -1;
    int rg = 0, rd = 0;
    ...
    if (opt_d)
        rd = mlx_faciesgan_optimize_discriminator_from_ag(m, opt_d, disc_params, disc_n);
    if ((opt_g && rg != 0) || (opt_d && rd != 0))
        return -1;
    return 0;
}

int mlx_faciesgan_set_shapes(MLXFaciesGAN *m, const int *shapes_flat, int n_scales)
{
    if (!m)
        return -1;
    if (m->shapes_flat)
...
int mlx_faciesgan_set_noise_amps(MLXFaciesGAN *m, const float *amps, int n)
{
    ...
    if (!amps || n <= 0)
        return 0;
    m->noise_amps = (float *)malloc(sizeof(float) * n);
    if (!m->noise_amps)
        return -1;
...
int mlx_faciesgan_get_shapes_flat(MLXFaciesGAN *m, int **out_shapes_flat, int *out_n_scales)
{
...
int mlx_faciesgan_get_noise_amps(MLXFaciesGAN *m, float **out_amps, int *out_n)
{
    ...
    float *copy = (float *)malloc(sizeof(float) * m->n_noise_amps);
    if (!copy)
        return -1;
...
int mlx_faciesgan_get_noise_amplitude(MLXFaciesGAN *m, int scale, float **out_amps, int *out_n)
{
    if (!m || !out_amps || !out_n)
        return -1;
    if (!m->noise_amps || m->n_noise_amps == 0)
...
int mlx_faciesgan_get_pyramid_noise(MLXFaciesGAN *m, int scale, const int *indexes, int n_indexes, mlx_array ***out_noises, int *out_n, mlx_array **wells_pyramid, mlx_array **seismic_pyramid, int rec)
{
    (void)indexes;
    (void)n_indexes;
    (void)wells_pyramid;
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        int cond_present = (indexes && n_indexes > 0 && ((wells_pyramid && wells_pyramid[i]) || (seismic_pyramid && seismic_pyramid[i]))) ? 1 : 0;
        int c = 0;
        ...
        mlx_stream s = mlx_default_cpu_stream_new();
        mlx_array *a = (mlx_array *)malloc(sizeof(mlx_array));
        if (!a)
        {
            for (int j = 0; j < n; ++j)
            {
        ...
        /* base noise */
        if (mlx_random_normal(a, shape, 4, MLX_FLOAT32, 0.0f, 1.0f, mlx_array_empty, s) != 0)
        {
        ...
        /* If conditioning provided, slice by `indexes` and concat with noise along channel axis.
           Support both wells and seismic concatenation (noise, wells, seismic). */
        if (indexes && n_indexes > 0 && wells_pyramid && wells_pyramid[i] && seismic_pyramid && seismic_pyramid[i])
        {
            ...
            if (mlx_gather_single(&wells_sel, *wells_pyramid[i], idx, 0, NULL, 0, s) == 0 && mlx_gather_single(&seismic_sel, *seismic_pyramid[i], idx, 0, NULL, 0, s) == 0)
            {
                mlx_vector_array vec = mlx_vector_array_new_data((const mlx_array[]){*a, wells_sel, seismic_sel}, 3);
        ...
        else if (indexes && n_indexes > 0 && wells_pyramid && wells_pyramid[i])
        {
            ...
            if (mlx_gather_single(&cond_sel, *wells_pyramid[i], idx, 0, NULL, 0, s) == 0)
            {
                /* concat noise and cond_sel */
                mlx_vector_array vec = mlx_vector_array_new_data((const mlx_array[]){*a, cond_sel}, 2);
                mlx_array znew = mlx_array_new();
                if (mlx_concatenate_axis(&znew, vec, 3, s) == 0)
                {
        ...
        else if (indexes && n_indexes > 0 && seismic_pyramid && seismic_pyramid[i])
        {
            int idx_shape[1] = {n_indexes};
            mlx_array idx = mlx_array_new_data(indexes, idx_shape, 1, MLX_INT32);
            mlx_array cond_sel = mlx_array_new();
            if (mlx_gather_single(&cond_sel, *seismic_pyramid[i], idx, 0, NULL, 0, s) == 0)
            {
                mlx_vector_array vec = mlx_vector_array_new_data((const mlx_array[]){*a, cond_sel}, 2);
                mlx_array znew = mlx_array_new();
                if (mlx_concatenate_axis(&znew, vec, 3, s) == 0)
        ...
        if (mlx_pad(&padded, *a, axes, 2, low_pad, 2, high_pad, 2, pad_val, "constant", s) == 0)
        {
            if (a->ctx)
                mlx_array_free(*a);
            *a = padded;
...
int mlx_faciesgan_save_generator_state(MLXFaciesGAN *m, const char *scale_path, int scale)
{
    if (!m || !scale_path)
        return -1;
    if (!m->generator)
...
int mlx_faciesgan_save_discriminator_state(MLXFaciesGAN *m, const char *scale_path, int scale)
{
    if (!m || !scale_path)
        return -1;
    if (!m->discriminator)
...
int mlx_faciesgan_load_generator_state(MLXFaciesGAN *m, const char *scale_path, int scale)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (mlx_map_string_to_array_get(&tmp, map, key) == 0)
        {
            ...
            (void)mlx_array_set(plist[i], tmp);
        }
    }
...
int mlx_faciesgan_load_discriminator_state(MLXFaciesGAN *m, const char *scale_path, int scale)
{
...
int mlx_faciesgan_save_shape(MLXFaciesGAN *m, const char *scale_path, int scale)
{
    if (!m || !scale_path)
        return -1;
...
int mlx_faciesgan_load_shape(MLXFaciesGAN *m, const char *scale_path, int scale)
{
    if (!m || !scale_path)
        return -1;
    char file[1024];
    ...
    if (mlx_map_string_to_array_get(&arr, map, "shape") != 0)
    {
        ...
        mlx_map_string_to_array_iterator it = mlx_map_string_to_array_iterator_new(map);
    ...
    if (elems >= 3)
    {
        ...
        if (data)
        {
            ...
            int *newflat = (int *)realloc(m->shapes_flat, sizeof(int) * (m->n_shapes + 1) * 4);
            if (!newflat)
            {
                mlx_map_string_to_array_free(map);
                mlx_map_string_to_string_free(meta);
...
int mlx_faciesgan_load_amp(MLXFaciesGAN *m, const char *scale_path)
{
    ...
    if (fscanf(f, "%lf", &v) == 1)
    {
        float *na = (float *)realloc(m->noise_amps, sizeof(float) * (m->n_noise_amps + 1));
        if (!na)
        {
            fclose(f);
            return -1;
...
int mlx_faciesgan_optimize_discriminator_scales(MLXFaciesGAN *m, const int *indexes, int n_indexes, MLXOptimizer **optimizers_by_scale, mlx_array **facies_pyramid, mlx_array **wells_pyramid, mlx_array **seismic_pyramid, const int *active_scales, int n_active_scales)
{
    if (!m)
        return -1;
    (void)indexes;
    ...
    for (int i = 0; i < n_active_scales; ++i)
    {
        ...
        /* Placeholder backward: create zero scalar and backward to produce zero grads. */
        ...
        */
        ...
        int gen_n_noises = 0;
        if (mlx_faciesgan_get_pyramid_noise(m, scale, indexes, n_indexes, &gen_noises, &gen_n_noises, wells_pyramid, seismic_pyramid, 0) == 0)
        {
            if (gen_n_noises > scale && gen_noises[scale])
            {
                noise_in = ag_value_from_array(gen_noises[scale], 0);
        ...
        AGValue *fake = mlx_faciesgan_generator_forward_ag(m, NULL, 0, NULL, 0, noise_in, scale, scale);
        if (fake)
        ...
        /* Gradient penalty: sample interpolation between real and fake, compute ||grad_x D(x_hat)||_2 and penalize (||.||-1)^2 */
        float gp_lambda = 10.0f;
        if (real_in && fake)
        {
            /* eps = 0.5 (broadcastable) for simplicity; production should randomize per-sample */
            ...
            AGValue *d_interp = mlx_faciesgan_discriminator_forward_ag(m, interp, scale);
            if (d_interp)
                ag_register_temp_value(d_interp);

            /* Build create_graph grads: grad of d_interp wrt interp as AGValue->grad_ag */
            if (d_interp)
            {
                if (mlx_faciesgan_get_use_create_graph_gp())
                {
                    ...
                    if (g)
                    {
                        ...
                        /* sum over H,W,channel axes. Determine shape dims via underlying array */
                ...
                {
                    /* Numeric fallback: compute numeric grads for d_interp, form GP scalar, add as constant. */
                    ...
                    /* clear numeric grads produced by this temporary backward so later ag_backward(loss) is clean */
                    ag_zero_grad_all();
                }
            }
        }
...
int mlx_faciesgan_optimize_generator_scales(MLXFaciesGAN *m, const int *indexes, int n_indexes, MLXOptimizer **optimizers_by_scale, mlx_array **facies_pyramid, mlx_array **rec_in_pyramid, mlx_array **wells_pyramid, mlx_array **masks_pyramid, mlx_array **seismic_pyramid, const int *active_scales, int n_active_scales)
{
    if (!m)
        return -1;
    (void)indexes;
    ...
    for (int i = 0; i < n_active_scales; ++i)
    {
        ...
        if (fake && masks_pyramid && masks_pyramid[scale] && rec_in_pyramid && rec_in_pyramid[scale])
        {
            mlx_array *num_loss = NULL;
            if (mlx_faciesgan_compute_masked_loss(m, ag_value_array(fake), rec_in_pyramid[scale], (wells_pyramid ? wells_pyramid[scale] : NULL), masks_pyramid[scale], 10.0f, &num_loss) == 0 && num_loss)
            {
                mlx_array arr = *num_loss;
                mlx_array_eval(arr);
                float v = 0.0f;
                (void)mlx_array_item_float32(&v, arr);
                printf("[C-masked-loss] scale=%d value=%f\n", scale, v);
                mlx_array_free(arr);
                free(num_loss);
            }
        }

        if (rec_in_pyramid && rec_in_pyramid[scale] && facies_pyramid && facies_pyramid[scale])
        {
            mlx_array *rec_loss_num = NULL;
            if (mlx_faciesgan_compute_recovery_loss(m, NULL, 0, scale, *rec_in_pyramid[scale], *facies_pyramid[scale], wells_pyramid, seismic_pyramid, 1.0f, &rec_loss_num) == 0 && rec_loss_num)
            {
                mlx_array arr = *rec_loss_num;
                mlx_array_eval(arr);
                float v = 0.0f;
                (void)mlx_array_item_float32(&v, arr);
                printf("[C-recovery-loss] scale=%d value=%f\n", scale, v);
                mlx_array_free(arr);
                free(rec_loss_num);
            }
        }
...
int mlx_faciesgan_compute_recovery_loss(MLXFaciesGAN *m, const int *indexes, int n_indexes, int scale, const mlx_array rec_in, const mlx_array real, mlx_array **wells_pyramid, mlx_array **seismic_pyramid, float alpha, mlx_array **out_loss)
{
    (void)indexes;
    (void)n_indexes;
    (void)wells_pyramid;
...
AGValue *mlx_faciesgan_compute_recovery_loss_ag(MLXFaciesGAN *m, const int *indexes, int n_indexes, int scale, AGValue *rec_in, AGValue *real, mlx_array **wells_pyramid, mlx_array **seismic_pyramid, float alpha)
{
    (void)m;
    (void)indexes;
    (void)n_indexes;
