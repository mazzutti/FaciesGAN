...
struct MLXDiscriminator {
    ...
    mlx_dtype dtype;
    int eval_mode;
    /* Cached all-params list — invalidated by create_scale */
    mlx_array **cached_params;
    int         cached_params_n;
};

MLXDiscriminator *mlx_discriminator_create(int num_layer, int kernel_size,
        int padding_size,
        int input_channels) {
    MLXDiscriminator *m = NULL;
    if (mlx_alloc_pod((void **)&m, sizeof(MLXDiscriminator), 1) != 0)
        return NULL;
...
void mlx_discriminator_free(MLXDiscriminator *m) {
    ...
    if (m->discs) {
        for (int i = 0; i < m->n_discs; ++i) {
            if (m->discs[i])
                mlx_spadedisc_free((MLXSPADEDiscriminator *)m->discs[i]);
        }
        mlx_free_pod((void **)&m->discs);
    }
    mlx_free_pod((void **)&m);
}

int mlx_discriminator_create_scale(MLXDiscriminator *m, int num_features,
                                   int min_num_features) {
    if (!m)
        return -1;
    /* Invalidate cached parameter list — new scale changes parameter set */
    if (m->cached_params) {
        mlx_free_pod((void **)&m->cached_params);
        m->cached_params = NULL;
        m->cached_params_n = 0;
    }
    MLXSPADEDiscriminator *disc =
        mlx_spadedisc_create(num_features, min_num_features, m->num_layer,
                             m->kernel_size, m->padding_size, m->input_channels);
    if (!disc)
        return -1;
    /* propagate dtype if the sub-discriminator supports it (best-effort) */
    /* If mlx_spadedisc_set_dtype exists we would call it here. For now just store
     * on parent. */
    void **tmp =
        (void **)realloc(m->discs, (size_t)(m->n_discs + 1) * sizeof(void *));
    if (!tmp) {
...
mlx_array_t mlx_discriminator_forward(MLXDiscriminator *m, int scale,
                                      mlx_array_t input) {
    if (!m)
        return input;
    if (scale < 0 || scale >= m->n_discs)
        return input;
...
mlx_array **mlx_discriminator_get_parameters(MLXDiscriminator *m,
        int *out_count) {
    if (!m || !out_count)
        return NULL;
...
void mlx_discriminator_free_parameters_list(mlx_array **list) {
    /* No-op: list is now cached on the struct and freed in mlx_discriminator_free */
    (void)list;
}
...
mlx_array_t mlx_discriminator_call(MLXDiscriminator *m, int scale,
                                   mlx_array_t input) {
    return mlx_discriminator_forward(m, scale, input);
}

/* Eval mode stub: enables/disables evaluation behavior (e.g., batch-norm stat
 * use). */
