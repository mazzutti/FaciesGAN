...
static void fg_create_generator_scale(void *mgr, int scale, int num_feature, int min_num_feature);
static void fg_create_discriminator_scale(void *mgr, int num_feature, int min_num_feature);
...
MLXBaseManager *mlx_base_manager_create_with_faciesgan(const MLXTrainOptions *opts)
{
    ...
    MLXFaciesGAN *fg = mlx_faciesgan_create(
        ...
        opts->kernel_size,
        opts->padding_size,
        opts->num_img_channels,
        opts->num_feature,
        opts->min_num_feature,
        opts->discriminator_steps,
        opts->generator_steps);
    if (!fg)
        return NULL;
