...
int mlx_base_manager_add_shape(MLXBaseManager *mgr, const int *shape,
  ...
  size_t *nndim =
      realloc(mgr->shapes_ndim, sizeof(size_t) * (mgr->shapes_count + 1));
  if (!nshapes || !nndim)
    return -1;
  mgr->shapes = nshapes;
...
void mlx_base_manager_init_generator_for_scale(MLXBaseManager *mgr, int scale) {
  ...
  /* mark active */
  int *tmp =
      realloc(mgr->active_scales, sizeof(int) * (mgr->active_scales_count + 1));
  if (tmp) {
    ...
    if (access(ampfile, F_OK) == 0) {
      if (mgr->cbs.load_amp)
        ...
        if (f) {
          double v = 1.0;
          if (fscanf(f, "%lf", &v) == 1) {
            double *na = realloc(mgr->noise_amps,
                                 sizeof(double) * (mgr->noise_amps_count + 1));
            if (na) {
              mgr->noise_amps = na;
