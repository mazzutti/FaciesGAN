...
void *mlx_nn_instancenorm_create(int num_features, int affine) {
  mlx_nn_instancenorm *h =
      (mlx_nn_instancenorm *)malloc(sizeof(mlx_nn_instancenorm));
  if (!h)
    return NULL;
...
mlx_array_t mlx_upsample_forward(MLXUpsample *m, mlx_array_t x) {
  ...
  if (m->out_h % in_h != 0 || m->out_w % in_w != 0) {
    /* Non-integer scale factors not supported by this simple repeat-based
     * upsample */
    return x;
  }
...
mlx_array_t mlx_spade_forward(MLXSPADE *m, mlx_array_t x,
...

  {
    ...
    if (out_ndim == 4) {
      ...
      fprintf(stderr,
              "[spade_debug] spade_forward out ndim=%d shape=(%d,%d,%d,%d)\n",
              out_ndim, osh[0], osh[1], osh[2], osh[3]);
    }
    return out;
  }
}
...
mlx_array_t mlx_spadeconv_forward(MLXSPADEConvBlock *m, mlx_array_t x,
  ...
  /* debug: print spadeconv intermediate y shape */
  {
    ...
    if (y_ndim == 4) {
      ...
      fprintf(stderr,
              "[spade_debug] spadeconv after_spade ndim=%d shape=(%d,%d,%d,%d) "
              "in_ch=%d out_ch=%d\n",
              y_ndim, ysh[0], ysh[1], ysh[2], ysh[3], m->in_ch, m->out_ch);
    }
  }
  /* 2) Activation */
  ...
  if (m->conv) {
    ...
    /* debug: print conv output shape */
    {
      ...
      if (out_ndim == 4) {
        const int *osh = mlx_array_shape(out);
        fprintf(stderr,
                "[spade_debug] spadeconv conv_out ndim=%d shape=(%d,%d,%d,%d) "
                "in_ch=%d out_ch=%d\n",
                out_ndim, osh[0], osh[1], osh[2], osh[3], m->in_ch, m->out_ch);
      }
    }
...
mlx_array_t mlx_spadegen_forward(MLXSPADEGenerator *m, mlx_array_t cond) {
  ...
  if (m->init_conv) {
    ...
    /* debug: print init conv output shape */
    {
      ...
      if (x_ndim == 4) {
        ...
        fprintf(
            stderr,
            "[spade_debug] spadegen init_conv ndim=%d shape=(%d,%d,%d,%d)\n",
            x_ndim, xsh[0], xsh[1], xsh[2], xsh[3]);
      }
    }
  } else {
  ...
  /* debug: print after leakyrelu */
  {
    ...
    if (x_ndim == 4) {
      ...
      fprintf(
          stderr,
          "[spade_debug] spadegen after_leaky ndim=%d shape=(%d,%d,%d,%d)\n",
          x_ndim, xsh[0], xsh[1], xsh[2], xsh[3]);
    }
  }
  ...
  if (m->spade_blocks) {
    for (int i = 0; i < m->n_blocks; ++i) {
      ...
      /* debug: print block output shape */
      {
        int nx_ndim = mlx_array_ndim(nx);
        if (nx_ndim == 4) {
          const int *nxsh = mlx_array_shape(nx);
          fprintf(stderr,
                  "[spade_debug] spadegen block %d out ndim=%d "
                  "shape=(%d,%d,%d,%d)\n",
                  i, nx_ndim, nxsh[0], nxsh[1], nxsh[2], nxsh[3]);
        }
      }
      if (x.ctx != nx.ctx)
        mlx_array_free(x);
  ...
  if (m->tail_conv) {
    ...
    /* debug: print tail output shape */
    {
      int ot_ndim = mlx_array_ndim(out_t);
      if (ot_ndim == 4) {
        const int *otsh = mlx_array_shape(out_t);
        fprintf(
            stderr,
            "[spade_debug] spadegen tail out_t ndim=%d shape=(%d,%d,%d,%d)\n",
            ot_ndim, otsh[0], otsh[1], otsh[2], otsh[3]);
      }
    }
    mlx_array_free(out);
    if (x.ctx != cond.ctx)
...
MLXSPADEDiscriminator *mlx_spadedisc_create(int num_features,
                                            ...
                                            int input_channels) {
  MLXSPADEDiscriminator *m =
      (MLXSPADEDiscriminator *)malloc(sizeof(MLXSPADEDiscriminator));
  if (!m)
    return NULL;
  m->head = NULL;
  int body_count = (num_layer - 2) > 0 ? (num_layer - 2) : 0;
  ...
  if (body_count > 0) {
    ...
    for (int i = 0; i < body_count; ++i) {
      ...
      m->body[i] = (void *)mlx_spadeconv_create(
...
MLXSPADEConvBlock *mlx_spadedisc_get_body_at(MLXSPADEDiscriminator *m,
                                             int idx) {
  if (!m)
    return NULL;
...
MLXColorQuantization *mlx_colorquant_create(float temperature) {
  MLXColorQuantization *m =
      (MLXColorQuantization *)malloc(sizeof(MLXColorQuantization));
  if (!m)
    return NULL;
