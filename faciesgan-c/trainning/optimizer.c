...
{
    MLXOptimizer *o = (MLXOptimizer *)malloc(sizeof(MLXOptimizer));
    if (!o)
        return NULL;
    o->lr = lr;
    o->beta1 = beta1;
...
int mlx_adam_step(MLXOptimizer *opt, mlx_array **params, mlx_array **grads, int n)
{
    ...
    {
        float tmp_lr[4];
        int got = mlx_scheduler_lr_for_step(opt->attached_scheduler, opt->step, tmp_lr, 1);
        if (got > 0)
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        mlx_array scal_b1 = mlx_array_new_float(opt->beta1);
        mlx_array scal_1mb1 = mlx_array_new_float(1.0f - opt->beta1);
        ...
        mlx_array scal_b2 = mlx_array_new_float(opt->beta2);
        mlx_array scal_1mb2 = mlx_array_new_float(1.0f - opt->beta2);
        ...
        {
            float decay_scale = opt->lr * opt->weight_decay;
            mlx_array decay_scale_arr = mlx_array_new_float(decay_scale);
...
void mlx_optimizer_init_from_params(MLXOptimizer *opt, mlx_array **params, int n)
{
    ...
    mlx_stream s = mlx_default_cpu_stream_new();
    opt->param_n = n;
    opt->m = (mlx_array **)malloc(sizeof(mlx_array *) * n);
    opt->v = (mlx_array **)malloc(sizeof(mlx_array *) * n);
    if (!opt->m || !opt->v)
    {
        if (opt->m)
            free(opt->m);
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if (params[i])
        {
            ...
            {
                mlx_array *mp = (mlx_array *)malloc(sizeof(mlx_array));
                if (mp)
                    *mp = tmp;
                opt->m[i] = mp;
                mlx_array tmp2 = mlx_array_new();
                ...
                {
                    mlx_array *vp = (mlx_array *)malloc(sizeof(mlx_array));
                    if (vp)
                        *vp = tmp2;
                    opt->v[i] = vp;
                }
...
/* Simple in-memory writer used to capture .npy bytes written by mlx_save_writer. */
...
static int serialize_array_to_npy_bytes(const mlx_array arr, void **out_buf, size_t *out_size)
{
    if (!out_buf || !out_size)
        return -1;
    size_t nbytes = mlx_array_nbytes(arr);
    size_t bufsize = nbytes + 4096; /* generous header space */
    mlx_mem_stream *m = (mlx_mem_stream *)malloc(sizeof(*m));
    if (!m)
        return -1;
...
int mlx_optimizer_save_to_npz(MLXOptimizer *opt, const char *npz_path)
{
    ...
    int n_members = n_params * 2 + 7; /* m/v pairs + step,lr,beta1,beta2,eps,weight_decay,bias_correction */
    const char **names = (const char **)malloc(sizeof(char *) * n_members);
    const void **bufs = (const void **)malloc(sizeof(void *) * n_members);
    ...
    for (int i = 0; i < n_params; ++i)
    {
        ...
        void *buf = NULL;
        size_t sz = 0;
        if (opt->m && opt->m[i] && serialize_array_to_npy_bytes(*opt->m[i], &buf, &sz) == 0)
        {
            bufs[idx] = buf;
            sizes[idx] = sz;
            idx++;
        ...
        buf = NULL;
        sz = 0;
        if (opt->v && opt->v[i] && serialize_array_to_npy_bytes(*opt->v[i], &buf, &sz) == 0)
        {
            bufs[idx] = buf;
            sizes[idx] = sz;
            idx++;
    ...
    /* scalar entries: step (int64), lr, beta1, beta2, eps, weight_decay (float32), bias_correction (int32) */
    /* step */
    names[idx] = "step.npy";
    {
...
int mlx_optimizer_load_from_npz(MLXOptimizer *opt, const char *npz_path)
{
    ...
    /* scalars: step, lr, beta1, beta2, eps, weight_decay, bias_correction */
    {
        ...
        if (npz_extract_member_to_mlx_reader(npz_path, "weight_decay.npy", &reader) == 0)
        {
            mlx_array tmp = mlx_array_new();
            if (mlx_load_reader(&tmp, reader, s) == 0)
            {
        ...
        if (npz_extract_member_to_mlx_reader(npz_path, "bias_correction.npy", &reader) == 0)
        {
            mlx_array tmp = mlx_array_new();
            if (mlx_load_reader(&tmp, reader, s) == 0)
            {
