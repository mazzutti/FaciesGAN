    ...
    int checkpoint_every)
{
    ...
    for (int e = 0; e < epochs; ++e)
    {
        for (int s = 0; s < steps_per_epoch; ++s)
        {
            ...
            {
                float tmp_lr[16];
                (void)mlx_scheduler_step_and_get_lr(sched, step_idx, opt_g, tmp_lr, sizeof(tmp_lr) / sizeof(tmp_lr[0]));
            ...
            int disc_n = 0;
            float loss = 0.0f;
            int rc = cb(mgr, step_idx, cb_ctx, &gen_grads, &gen_n, &disc_grads, &disc_n, &loss);
            if (rc != 0)
                return -1;
            ...
            if (opt_d && disc_n > 0)
                r2 = mlx_base_apply_sgd_to_discriminator(mgr, opt_d, disc_grads, disc_n);

            /* logging */
            if (logger)
            {
                char buf[256];
                snprintf(buf, sizeof(buf), "epoch=%d step=%d loss=%f rg=%d rd=%d", e, s, loss, r1, r2);
            ...
            /* checkpointing */
            if (checkpoint_path && checkpoint_every > 0 && ((step_idx + 1) % checkpoint_every == 0))
            {
                ...
                if (fg)
                {
                     ...
                     * - Create per-scale directories under the checkpoint's parent directory
                        ...
                        for (int si = 0; si < nscales; ++si)
                        {
                                ...
                                if (sp)
                                {
                                    ...
                                    for (int pi = 0; pi < pcount; ++pi)
                                    {
                                        if (!plist[pi])
                                            continue;
                                        char fname[PATH_MAX];
                                        snprintf(fname, PATH_MAX, "%s/param_spade_%d.npy", scale_dir, pi);
                            ...
                            else
                            {
                                ...
                                for (int bi = 0; bi < bc; ++bi)
                                {
                                    ...
                                    {
                                        char fname[PATH_MAX];
                                        snprintf(fname, PATH_MAX, "%s/param_body_%d.npy", scale_dir, bi);
                            ...
                            char cmd[PATH_MAX * 2];
                            snprintf(cmd, sizeof(cmd), "python3 '/Users/mazzutti/POSDOC/Experimentos/FaciesGAN/trainning/mlx-c/pack_scale_npz.py' '%s'", scale_dir);
