 ...
 * [version int32]
 * [gen_count int32]
 * for each gen param: dtype(int32), ndim(int32), shape(int32*ndim), nbytes(int64), data(bytes)
 * [disc_count int32]
 * for each disc param: same format
 */
...
int mlx_checkpoint_save(const char *path, MLXFaciesGAN *m)
{
    ...
    {
        if (mlx_faciesgan_get_shapes_flat(m, &shapes, &n_shapes) == 0 && shapes && n_shapes > 0)
        {
...
int mlx_checkpoint_load(const char *path, MLXFaciesGAN *m)
{
    ...
    if (n_shapes > 0)
    {
        int *shapes = (int *)malloc(sizeof(int) * n_shapes * 4);
        if (!shapes)
        {
            fclose(f);
            return -1;
        }
        if (fread(shapes, sizeof(int), n_shapes * 4, f) != (size_t)(n_shapes * 4))
        {
            free(shapes);
            fclose(f);
            return -1;
    ...
    if (n_amps > 0)
    {
        float *amps = (float *)malloc(sizeof(float) * n_amps);
        if (!amps)
        {
            fclose(f);
            return -1;
