/* Stub for example_train_from_ag.c — minimal entry so CMake can build examples during parity checks */
#include <stdio.h>
...
static void copy_host_data_of_array(const mlx_array *a, float **out_buf, size_t *out_count)
{
    if (!a)
    {
        *out_buf = NULL;
...
int main(void)
{
    ...
    opts.num_feature = 16;
    opts.min_num_feature = 8;
    opts.num_layer = 1;
    opts.kernel_size = 3;
    opts.padding_size = 1;
    opts.num_img_channels = 1;
    opts.discriminator_steps = 1;
    opts.generator_steps = 1;
    ...
    for (int step = 0; step < num_steps; ++step)
    {
        ...
        int r = mlx_base_train_step_from_ag(mgr, opt_g, ag_params, param_n, NULL, NULL, 0);
        if (r != 0)
    ...
    {
        printf("first-param before[0]=%f after[0]=%f\n", before_buf[0], after_buf[0]);
    }

    /* Try to save checkpoint and per-scale generator.npz into OUTPUT_PATH if provided. */
                ...
                for (int si = 0; si < nscales; ++si)
                {
                    ...
                    if (mlx_generator_has_spade_scale(g2, si))
                    {
                        ...
                        if (sp)
                        {
                            ...
                            for (int pi = 0; pi < pcount; ++pi)
                            {
                                ...
                                char fname[1024];
                                if (pi == 0)
                                    snprintf(fname, sizeof(fname), "%s/init_conv.weight.npy", scale_dir);
                                else if (pi == pcount - 1)
                                    snprintf(fname, sizeof(fname), "%s/tail_conv.weight.npy", scale_dir);
                                else
                                    snprintf(fname, sizeof(fname), "%s/spade_blocks.layers.%d.conv.weight.npy", scale_dir, pi - 1);
                    ...
                    else
                    {
                        ...
                        if (head)
                        {
                            ...
                            {
                                char fname[1024];
                                snprintf(fname, sizeof(fname), "%s/head.conv.weight.npy", scale_dir);
                        ...
                        for (int bi = 0; bi < bc; ++bi)
                        {
                            ...
                            {
                                char fname[1024];
                                snprintf(fname, sizeof(fname), "%s/body.layers.%d.conv.weight.npy", scale_dir, bi);
                        ...
                        if (mlx_scale_has_tail_conv(g2, si))
                        {
                            ...
                            {
                                char fname[1024];
                                snprintf(fname, sizeof(fname), "%s/tail.layers.0.weight.npy", scale_dir);
                                (void)mlx_save(fname, *tw);
                            }
                        }
                    }

                    /* pack into generator.npz using helper script; optionally provide Python template NPZ and scale index */
                    const char *py_tpl = getenv("PYTHON_GOLD_NPZ");
                    char pack_cmd[4096];
                    if (py_tpl && py_tpl[0])
                    {
                        snprintf(pack_cmd, sizeof(pack_cmd), "python3 '%s/trainning/mlx-c/pack_scale_npz.py' '%s' '%s' %d", "/Users/mazzutti/POSDOC/Experimentos/FaciesGAN", scale_dir, py_tpl, si);
                    }
                    else
                    {
                        snprintf(pack_cmd, sizeof(pack_cmd), "python3 '%s/trainning/mlx-c/pack_scale_npz.py' '%s'", "/Users/mazzutti/POSDOC/Experimentos/FaciesGAN", scale_dir);
                    }
