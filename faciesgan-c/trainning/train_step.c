...
int mlx_base_train_step(MLXBaseManager *mgr, MLXOptimizer *opt_g, mlx_array **gen_grads, int gen_n, MLXOptimizer *opt_d, mlx_array **disc_grads, int disc_n)
{
    ...
    if (!fg)
        return -1;
    return mlx_faciesgan_train_step(fg, opt_g, gen_grads, gen_n, opt_d, disc_grads, disc_n);
}
...
int mlx_base_train_step_from_ag(MLXBaseManager *mgr, MLXOptimizer *opt_g, AGValue **gen_params, int gen_n, MLXOptimizer *opt_d, AGValue **disc_params, int disc_n)
{
    ...
    if (opt_d)
        rd = mlx_base_apply_sgd_to_discriminator_from_ag(mgr, opt_d, disc_params, disc_n);
    if ((opt_g && rg != 0) || (opt_d && rd != 0))
        return -1;
    return 0;
}
    ...
    MLXResults **out_results)
{
    ...
    MLXResults *res = (MLXResults *)malloc(sizeof(MLXResults));
    if (!res)
        return -1;
    res->n_scales = n_active_scales;
    res->scales = (MLXScaleResults *)calloc(n_active_scales, sizeof(MLXScaleResults));
    if (!res->scales)
    {
        free(res);
        return -1;
    ...
    for (int si = 0; si < n_active_scales; ++si)
    {
        ...
        AGValue **gen_params_ag = (AGValue **)malloc(sizeof(AGValue *) * gen_param_n);
        if (!gen_params_ag)
        {
            mlx_generator_free_parameters_list(gen_params_list);
            continue;
        ...
        if (d_real && d_fake)
        {
            AGValue *gp = mlx_faciesgan_compute_gradient_penalty_ag(m, real, fake, scale, lambda_grad);
            if (gp)
            {
        ...
        if (disc_loss)
        {
            ...
            {
                AGValue **disc_params_ag = (AGValue **)malloc(sizeof(AGValue *) * disc_param_n);
        ...
        /* masked loss */
        if (fake && facies_pyramid[scale] && masks_pyramid && masks_pyramid[scale])
        {
            ...
            AGValue *masked = mlx_faciesgan_compute_masked_loss_ag(m, fake, real_ag, well_ag, mask_ag, well_loss_penalty);
            if (masked)
            {
        ...
        /* recovery loss: if rec_in_pyramid not provided, compute it from facies_pyramid */
        {
            ...
            if (facies_pyramid && facies_pyramid[scale])
            {
                ...
                {
                    if (mlx_compute_rec_input(scale, indexes, n_indexes, facies_pyramid, &tmp_rec) == 0 && tmp_rec)
                    {
                        use_rec = tmp_rec;
                        tmp_computed = 1;
                    }
            ...
            if (use_rec && facies_pyramid[scale])
            {
                ...
                if (tmp_computed)
                {
                    ...
                    (void)mlx_init_rec_noise_and_amp(m, scale, indexes, n_indexes, facies_pyramid[scale], wells_pyramid, seismic_pyramid);
                }
                AGValue *rec_in_ag = ag_value_from_array(use_rec, 0);
                ag_register_temp_value(rec_in_ag);
                AGValue *real_ag = ag_value_from_array(facies_pyramid[scale], 0);
                ag_register_temp_value(real_ag);
                AGValue *recv = mlx_faciesgan_compute_recovery_loss_ag(m, indexes, n_indexes, scale, rec_in_ag, real_ag, wells_pyramid, seismic_pyramid, alpha);
                if (recv)
                {
        ...
        if (n_div > 1)
        {
            AGValue **fake_samples = (AGValue **)malloc(sizeof(AGValue *) * n_div);
            if (fake_samples)
            {
                ...
                for (int di = 0; di < n_div; ++di)
                {
                    ...
                    int tmp_n = 0;
                    if (mlx_faciesgan_get_pyramid_noise(m, scale, indexes, n_indexes, &tmp_noises, &tmp_n, wells_pyramid, seismic_pyramid, 0) == 0)
                    {
                        if (tmp_n > scale && tmp_noises[scale])
                        {
                            AGValue *n_ag = ag_value_from_array(tmp_noises[scale], 0);
                            ag_register_temp_value(n_ag);
                            AGValue *f = mlx_faciesgan_generator_forward_ag(m, NULL, 0, NULL, 0, n_ag, scale, scale);
                            if (f)
                ...
                div_comp = mlx_faciesgan_compute_diversity_loss_ag(m, fake_samples, n_div, lambda_diversity);
                if (div_comp)
                {
...
void mlx_results_free(MLXResults *res)
{
    ...
    for (int i = 0; i < res->n_scales; ++i)
    {
        ...
        if (sr->metrics.fake)
        {
            ...
            free(sr->metrics.fake);
        }
        if (sr->metrics.well)
        {
            mlx_array_free(*sr->metrics.well);
            free(sr->metrics.well);
        }
        if (sr->metrics.div)
        {
