...
static void print_help(void) {
  ...
  printf("  -i <path>, --input-path <path>     Input data root (must contain "
         "%s/)\n",
         DF_DIR_FACIES);
  printf("  -o <path>, --output-path <path>    Output directory (default: "
         "results)\n");
  printf("      --output-fullpath <path>       Use exact output path (no "
         "timestamp)\n");
  ...
  printf("      --num-features <n>             Base feature count for "
         "generator\n");
  ...
  printf("      --discriminator-steps <n>      Discriminator steps per "
         "iteration\n");
  ...
  printf("      --num-generated-per-real <n>   Generated samples per real "
         "facies\n");
  printf(
      "      --wells-mask-columns c1,c2..   Columns to mask from well logs\n");
  printf(
      "      --use-wells                    Use well data during training\n");
  printf("      --well-loss-penalty <f>        Penalty weight for well-loss "
         "(ignored)\n");
  printf("      --use-seismic                  Use seismic data during "
         "training\n");
  printf("      --no-tensorboard               Disable tensorboard logging\n");
  printf("      --no-plot-facies               Disable facies plotting\n");
  printf("      --compile-backend              Compile C backend before "
         "running\n");
  printf(
      "      --use-mlx                      Use MLX backend instead of CUDA\n");
  printf("      --hand-off-to-c                Hand off execution to compiled "
         "C trainer\n");
}
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "hi:o:n:", long_options, NULL)) != -1) {
    ...
    case OPT_WELLS_MASK_COLUMNS: {
      ...
      if (tokbuf) {
        ...
        while (tok) {
          wells_mask_columns = (int *)realloc(
              wells_mask_columns, sizeof(int) * (wells_mask_count + 1));
  ...
  /* Transfer parsed wells mask columns into TrainningOptions for parity
   * with Python `options.wells_mask_columns`. Ownership of the
   * `wells_mask_columns` pointer is moved to `topt` so do not free it
   * afterwards. */
  topt->wells_mask_columns = wells_mask_columns;
  topt->wells_mask_count = wells_mask_count;
  ...
  printf("Launching C-native trainer (MLXTrainer API)...\n");

  /* Use the existing dataset-driven C runner which mirrors Python Trainer
   * initialization/selection/model/shapes behavior. This keeps the dataset
   * logic contained in `MLXTrainer_run_full`. */
  int rc = MLXTrainer_run_full(topt);
