...
int mlx_generate_scales_flat(MLXPyramidsDataset *ds, int channels_last) {
  ...
  int *flat = (int *)malloc(sizeof(int) * 4 * (size_t)n);
  if (!flat) {
    free(arr);
    return -1;
  }
      ...
      for (int sidx = 0; sidx < n_wells_scales; ++sidx) {
        ...
        if (wells_scales && mlx_array_ndim(wells_scales[sidx]) > 0) {
          ...
          if (wsh && wsh[0] > si) {
            ...
            if (wells_scales && mlx_array_ndim(wells_scales[sidx]) > 0) {
              int wnd = (int)mlx_array_ndim(wells_scales[sidx]);
              const int *wsh = mlx_array_shape(wells_scales[sidx]);
              (void)wsh;
            }
            int valid_wslice = 0;
      ...
      for (int sidx = 0; sidx < n_seis_scales; ++sidx) {
        ...
        if (seis_scales && mlx_array_ndim(seis_scales[sidx]) > 0) {
          ...
          if (ssh && ssh[0] > si) {
            ...
            if (seis_scales && mlx_array_ndim(seis_scales[sidx]) > 0) {
              int snd = (int)mlx_array_ndim(seis_scales[sidx]);
              const int *ssh = mlx_array_shape(seis_scales[sidx]);
              (void)ssh;
            }
            int valid_sslice = 0;
...
int mlx_pyramids_dataset_new(MLXPyramidsDataset **out,
  ...
  *out = NULL;
  ...
  (void)mlx_generate_scales_flat(ds, channels_last ? 1 : 0);
...
static int build_stack_from_source(mlx_vector_vector_array src,
  ...
  if (vec_n == 0) {
    ...
    mlx_vector_array free_tmp = mlx_vector_array_new();
    (void)free_tmp; /* silence unused warning when compiled out */
    mlx_vector_array_free(scale_vec);
    mlx_stream_free(s);
    mlx_vector_array sample0;
    if (mlx_vector_vector_array_get(&sample0, ds->facies, 0) != 0)
...
int mlx_pyramids_dataset_generate_pyramids(MLXPyramidsDataset *ds,
  ...
  mlx_array *fac = (mlx_array *)malloc(sizeof(mlx_array) * (size_t)n);
  mlx_array *wells = (mlx_array *)malloc(sizeof(mlx_array) * (size_t)n);
  mlx_array *seis = (mlx_array *)malloc(sizeof(mlx_array) * (size_t)n);
  if (!fac || !wells || !seis) {
    free(fac);
    free(wells);
    free(seis);
