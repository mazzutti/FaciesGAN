...
{
    if (!data_root || !subdir || !out_columns_tmp || !out_counts_tmp || !out_image_files || !out_image_count)
        return -1;
    char npz_path[PATH_MAX];
    if (find_first_npz(data_root, subdir, npz_path, sizeof(npz_path)) != 0)
        return -1;

    /* Try member names with extension (typical for .npz): columns.npy and counts.npy */
    char *cols_tmp = NULL;
    char *counts_tmp = NULL;
    mlx_io_reader cols_reader = {0};
    mlx_io_reader counts_reader = {0};
    /* Prefer in-memory extraction into MLX readers. Fall back to temp-file extraction if needed. */
    if (npz_extract_member_to_mlx_reader(npz_path, "columns.npy", &cols_reader) != 0)
    {
        if (npz_extract_member_to_mlx_reader(npz_path, "columns", &cols_reader) != 0)
            return -1;
    }
    if (npz_extract_member_to_mlx_reader(npz_path, "counts.npy", &counts_reader) != 0)
    {
        if (npz_extract_member_to_mlx_reader(npz_path, "counts", &counts_reader) != 0)
        {
    ...
    int img_count = 0;
    if (datasets_list_image_files(data_root, subdir, &image_list, &img_count) != 0)
    {
        free(cols_tmp);
        free(counts_tmp);
        return -1;
...
int datasets_load_wells_mapping(const char *data_root, const char *subdir,
                                ...
                                char ***out_image_files, int *out_image_count)
{
    if (!data_root || !subdir || !out_columns || !out_counts || !out_n || !out_image_files || !out_image_count)
        return -1;
    ...
    /* Try in-memory extraction first by locating the .npz and extracting into MLX readers. */
    char npz_path[PATH_MAX];
    if (find_first_npz(data_root, subdir, npz_path, sizeof(npz_path)) == 0)
    {
        if (npz_extract_member_to_mlx_reader(npz_path, "columns.npy", &cols_reader) != 0)
