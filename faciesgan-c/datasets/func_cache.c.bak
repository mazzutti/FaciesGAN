...
int ensure_function_cache(const char *input_path, const char *cache_dir,
    ...
    char hidden_cache_dir[PATH_MAX];
    /* Use a hidden subdirectory for cache files to avoid cluttering results */
    snprintf(hidden_cache_dir, PATH_MAX, "%s/.cache", cache_dir);
    /* Ensure hidden cache dir exists */
    struct stat __st_cacha = {0};
    if (stat(hidden_cache_dir, &__st_cacha) != 0) {
        if (mkdir(hidden_cache_dir, 0755) != 0 && errno != EEXIST) {
            /* Fall back to original cache_dir if creation failed */
            snprintf(hidden_cache_dir, PATH_MAX, "%s", cache_dir);
        }
    }
    snprintf(cache_npz, PATH_MAX, "%s/func_cache_%016llx.npz", hidden_cache_dir,
    ...
    int python_ok = 0;
    {
        ...
        if (rc == 0 && access(cache_npz, F_OK) == 0) {
            python_ok = 1;
            /* Python cache generator produced NPZ (info suppressed) */
        } else {
            /* Python cache generator not available — falling back to C generator */
        }
    }
    ...
    if (!python_ok) {
        ...
        int grc = generate_pyramids_cache(
                      input_path, hidden_cache_dir, num_samples, derived_stop, derived_crop,
    ...
    if (mlx_alloc_pod((void **)&names, sizeof(char *), n_members) != 0 ||
            mlx_alloc_pod((void **)&bufs, sizeof(void *), n_members) != 0 ||
            mlx_alloc_pod((void **)&sizes, sizeof(size_t), n_members) != 0) {
    ...
    for (int si = 0; si < num_samples; ++si) {
        ...
        snprintf(sample_dir, PATH_MAX, "%s/sample_%d", hidden_cache_dir, si);
        for (int sc = 0; sc < num_scales; ++sc) {
            ...
            if (!f) {
                ...
                for (int j = 0; j < idx; ++j) {
                    mlx_free_pod((void **)&names[j]);
                    mlx_free_pod((void **)&bufs[j]);
                }
                mlx_free_pod((void **)&names);
                mlx_free_pod((void **)&bufs);
                mlx_free_pod((void **)&sizes);
            ...
            if (fsize > INT_MAX) {
                ...
                if (mlx_alloc_pod(&buf, 1, (int)fsize) != 0)
                    buf = NULL;
            }
            if (!buf) {
                fclose(f);
            ...
            if (use_wells) {
                ...
                if (fw) {
                    ...
                    void *bufw = NULL;
                    if (fsizew > INT_MAX) {
                        bufw = malloc((size_t)fsizew);
                    } else {
                        if (mlx_alloc_pod(&bufw, 1, (int)fsizew) != 0)
                            bufw = NULL;
                    }
                    if (bufw) {
            ...
            if (use_seismic) {
                ...
                if (fs) {
                    ...
                    void *bufs_ = NULL;
                    if (fsizes > INT_MAX) {
                        bufs_ = malloc((size_t)fsizes);
                    } else {
                        if (mlx_alloc_pod(&bufs_, 1, (int)fsizes) != 0)
                            bufs_ = NULL;
                    }
                    if (bufs_) {
...
int generate_pyramids_cache(const char *input_path, const char *cache_dir,
    ...
    for (int si = 0; si < num_samples && si < count; ++si) {
        ...
        for (int scale = 0; scale <= stop_scale; ++scale) {
            ...
            if (target <= 0)
                target = min_size;
            size_t _nelem_uc = (size_t)target * target * num_img_channels;
            unsigned char *resized = NULL;
            if (_nelem_uc > (size_t)INT_MAX) {
                resized = malloc(_nelem_uc);
            } else {
                if (mlx_alloc_pod((void **)&resized, sizeof(unsigned char),
                                  (int)_nelem_uc) != 0)
                    resized = NULL;
            }
            if (!resized) {
            ...
            if (nelem > (size_t)INT_MAX) {
                fdata = (float *)malloc(nelem * sizeof(float));
            } else {
                if (mlx_alloc_float_buf(&fdata, (int)nelem) != 0)
                    fdata = NULL;
            }
            if (!fdata) {
                if (resized) {
