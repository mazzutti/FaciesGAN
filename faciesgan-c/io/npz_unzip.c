...
static void npz_mem_read_at_offset(void *ctx, char *data, size_t n, size_t off)
{
    struct npz_mem_desc *m = ctx;
    if (off >= m->size)
    {
...
/* Create an MLX IO reader that wraps an in-memory buffer extracted from an npz member. */
int npz_extract_member_to_mlx_reader(const char *npz_path, const char *member_name, mlx_io_reader *out_reader)
{
    ...
    struct npz_mem_desc *d = malloc(sizeof(*d));
    if (!d)
    {
        free(buf);
        return -1;
