...
def load_condfile(fname, skiprows=5):
    ...
    jj, ii, vals = data.T
    ij = np.asarray(list(zip(ii,jj)), dtype=int)
    return ij, vals

def scatter_ij(ax, ij, vals):
    ij0, ij1 = ij[np.where(vals<0)], ij[np.where(vals>0)]
    ax.scatter(ij0[:,1],ij0[:,0], marker='x', s=50, c='C1')
    ax.scatter(ij1[:,1],ij1[:,0], marker='o', s=50, c='C0')

def save_pic(condi_file,fake,outdir,scale,iter,index):
    img = fake.detach().cpu().numpy().squeeze()
    ...
    ax.set_ylim(img.shape[0]-1,0) #从左上角开始画
    ax.imshow(img,cmap='gray')
    if condi_file is not None:
        ij, vals = load_condfile(condi_file)
        ij=(ij*scale).astype(int)
        scatter_ij(ax, ij, vals)
    fig.savefig('{0}/GEN_{1}_{2}.png'.format(outdir,iter,index), bbox_inches='tight', pad_inches=0)
    plt.close(fig)

def gen_pos_and_weight(imgSize,file:str,radius:int=0,sqrt:bool=False):
    ''' imgsize: 图片大小 [imgSize,imgSize] 目前只支持二维
        file: 条件数据文件 Gslib格式 目前只支持0 1二值相类型
        radius: 距离反比的半径
        sqrt: False:距离的平方 True:距离
        文件中标记位置 0 插值为 -1 , 1 插值为 1 , 中间按照给定的距离半径在-1~1之间插值
        return: 返回索引列表[ [i,j] , [i,j] , ...]和[i,j]处的权重
    '''
    ...
    expand_vals=[]
    if radius>=0:
        for index in range(len(ij)):
            x=ij[index,0]
            y=ij[index,1]
            arri = np.linspace(x-radius,x+radius,radius*2+1).astype(int)
            arrj = np.linspace(y-radius,y+radius,radius*2+1).astype(int)
            for posindexi in range(len(arri)):
                for posindexj in range(len(arrj)):
                    x_=arri[posindexi]
                    y_=arrj[posindexj]
                    if(x_>=0 and x_< imgSize and y_>=0 and y_<imgSize):
                        if sqrt:
                            ratio2=np.sqrt((x-x_)*(x-x_)+(y-y_)*(y-y_)+1)
                        else:
                            ratio2=(x-x_)*(x-x_)+(y-y_)*(y-y_)+1
                        mask[x_,y_]=mask[x_,y_]+1.0/ratio2
                        if (x_,y_) in expand_ij:
                            clip=max(min(mask[x_,y_],1),0)
                            expand_vals[expand_ij.index((x_,y_))]=clip
                        else:
                            expand_ij.append((x_,y_))
                            clip=max(min(mask[x_,y_],1),0)
                            expand_vals.append(clip)
    return np.asarray(expand_ij, dtype=int),np.asarray(expand_vals,dtype=np.float32)

def weightandcondition(imgSize,file:str,radius:int=0,sqrt:bool=False):
    image_shape = [imgSize, imgSize]
    data = np.loadtxt(file, skiprows=5)
    jj, ii, vals = data.T
    vals=(vals-0.5)/1.0
    ...
    condi = np.zeros(image_shape,dtype=np.float32)
    if radius>=0:
        for index in range(len(ij)):
            x=ij[index,0]
            y=ij[index,1]
                ...
                for posindexj in range(len(arrj)):
                    ...
                    if(x_>=0 and x_< imgSize and y_>=0 and y_<imgSize):
                        if sqrt:
                            ...
                            ratio2=(x-x_)*(x-x_)+(y-y_)*(y-y_)+1
                        clip=min(weight[x_,y_]+1.0/ratio2,1)
                        weight[x_,y_]=clip
                        condi[x_,y_]=vals[index]
    return weight,condi